...
| **MicroPython** | Automatic for uncaught Python exceptions (`install_error_hook()`) | **Python traceback** (file/line/function) | Explicit — `report_log_error()` |
...
<Callout type="warn" title="Platform caveats — read before relying on these">
...
- **MicroPython automatic crash capture requires `sys.excepthook`** in your firmware build. `install_error_hook()` returns `False` when it is unavailable; on such builds you must report crashes explicitly.
