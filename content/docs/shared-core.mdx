...
Read it with `honch_core_get_last_error(client, &detail)` (copied out under the state lock; safe to keep). Format a one-line summary with `honch_error_detail_format(&detail, buf, sizeof(buf))`, e.g. `transport error: HTTP 401 - API key invalid or revoked (reason=auth_invalid_key)`. Reason tokens are available as stable strings via `honch_error_reason_string()`.
