...
Uploads are over TLS on every port. Certificate **verification** behavior differs by port — the table below is the source of truth:
...
| MicroPython | Connects over TLS but does **not** currently verify the server certificate (no pinning). This is a known limitation of the MicroPython transport — partly because constrained boards like the Pico W have no real-time clock to validate certificate validity periods against. Prefer a trusted network path and avoid sending sensitive data over an untrusted one on this port until verification lands. |
