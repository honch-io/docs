...
The Honch SDK runs directly on your device as part of your firmware. It collects events locally and sends them to the Honch platform in efficient batches over HTTPS. Events are queued on-device while connectivity is down, then sent once the network is available again.
...
- **Sessions**. You control these by calling `session_start()` and `session_end()`. All events between those calls share a session ID, making it easy to analyze user flows or operational sequences.
