...
<Callout title="Status">Stable · `0.2.4`. Reports `$sdk_platform` as `c-posix`.</Callout>
...
The build is warnings-as-errors. To compile out `$error`/fault-snapshot support, configure with `-DHONCH_ENABLE_ERROR_TRACKING=OFF`.
...
`honch_install_error_handlers(queue_directory)` installs async-signal-safe handlers for `SIGABRT`, `SIGSEGV`, `SIGBUS`, `SIGILL`, and `SIGFPE`. They write a bounded breadcrumb file; the next `honch_init()` imports it as an `$error` event. The path is process-global, so use it with a single client.
...
| `honch_report_error(client, &report, props, count)` | Queue an `$error`. |
| `honch_install_error_handlers(queue_directory)` | Install crash-breadcrumb signal handlers. |
