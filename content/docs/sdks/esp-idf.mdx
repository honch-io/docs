...
<Callout title="Status">Stable · `0.3.0`. Requires ESP-IDF `>= 5.0` (verified against v6.0.1).</Callout>
...
The `esp_http_client` transport records structured detail for every failed upload: the HTTP status maps to a `reason` (e.g. `401` → `auth_invalid_key`), and an `esp_err_t` connect/DNS/TLS failure maps into `reason` with the raw code in `os_error`. Read it with `honch_core_get_last_error(&detail)`. The core also emits one deduped line per distinct failure through the platform log hook under the `honch` tag — `ESP_LOGW` for retryable, `ESP_LOGE` for terminal — so `idf.py monitor` shows, for example:

```text
E (45231) honch: upload rejected: HTTP 401 - API key invalid or revoked (reason=auth_invalid_key)
```

Because the line is tagged `honch`, it is skipped by the `ESP_LOGE` → `$error` capture hook (no feedback loop). See [Error Context & Diagnostics](/shared-core#error-context--diagnostics).
