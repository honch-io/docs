...
| `enable_error_tracking` | `false` | Emit `$error` after an abnormal reset. See [Error Tracking](#error-tracking). |
...
| --- | --- | --- |
| `CONFIG_HONCH_ERROR_TRACKING` | on | Compiles in the boot fault-snapshot / `$error` path. |
| `CONFIG_HONCH_CRASH_SYMBOLICATION` | on | Adds firmware build IDs and raw crash addresses from ELF coredump summaries. Depends on error tracking. |

These control whether the code is *compiled in*. To actually emit a `$error` after an abnormal reset you must also set `enable_error_tracking = true` in your runtime config.

## Error Tracking

When `enable_error_tracking` is on, the SDK reads the reset reason during init and, for abnormal resets (panic, watchdog, brownout, and similar), queues a `$error` with the reset cause. With crash symbolication enabled and the coredump-to-flash options configured, a panic also carries a firmware build ID and fault address. Backtrace symbolication is available on Xtensa targets (the original ESP32 and S-series); RISC-V targets (C3/C6/H2) do not emit a backtrace field.
...
| `honch_report_error(&report, props, count)` | Queue an `$error`. |
