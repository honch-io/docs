...
The Honch ESP-IDF SDK is a C component for the [ESP-IDF](https://docs.espressif.com/projects/esp-idf/en/latest/esp32/) framework. It compiles alongside your application, queues events locally, and sends them to the configured Honch capture endpoint in efficient batches. Use the HTTPS Honch endpoint in production.
...
- **Wi-Fi connectivity**: the SDK sends events to the configured capture endpoint. It queues events locally when offline and flushes them once a network connection is available.
...
| `event_buffer` | Yes | - | A caller-owned byte array required by the current config API. Keep it `static` or global so it outlives `honch_init()`. |
| `event_buffer_size` | Yes | - | The size of `event_buffer` in bytes. The SDK README recommends >= 8192. |
...
| `battery_callback` | No | NULL | A function pointer the SDK calls to read the current battery level. It should return 0-100 (percentage) or -1 if unknown. When configured, the SDK stamps every event with `$battery_level` and emits `$battery_low` when the level is below `battery_low_threshold`. |
| `battery_low_threshold` | No | 15 | The battery percentage below which the SDK emits a `$battery_low` event. The event is emitted once while below this threshold, and not again until the level recovers to the threshold or above. |
...
| `CONFIG_HONCH_WIFI_RSSI_CACHE_MS` | 5000 | How long to cache Wi-Fi RSSI metadata while encoding events. Set to 0 to query RSSI for every event. |
| `CONFIG_HONCH_ENABLE_GZIP` | on where available | When enabled, the SDK compresses CBOR batches with gzip before sending. This option is disabled on ESP32 and ESP32-S2 because those targets do not expose the required miniz compressor symbol. If compression fails or doesn't reduce the payload size, the SDK falls back to sending raw CBOR. |
...
NVS must be set up before `honch_init()`:

1. **NVS (Non-Volatile Storage)**: ESP-IDF's key-value store in flash memory. The SDK uses NVS to persist the device ID and distinct ID across reboots. If NVS is not initialized, `honch_init()` fails with `HONCH_ERR_NVS`.

Wi-Fi does not need to be connected before `honch_init()`. The SDK queues events locally while offline and flushes after it observes an IP address. To receive connectivity lifecycle events, initialize the ESP event loop and Wi-Fi before or around SDK startup.
...
1. **Loads or stores a stable device ID**, derived from the chip's hardware MAC address and stored in NVS (format: `dev_` followed by 12 hex characters, e.g. `dev_a3f2c1d4e5f6`). On subsequent boots, the stored ID is loaded rather than regenerated, so the same physical device has the same ID.
2. **Initializes internal subsystems**: the RAM-first event queue with NVS spill, the HTTP transport, the GPIO tracking worker task, and the lifecycle event manager.
...
// are needed. Invalid JSON or non-object JSON is ignored. The SDK encodes the
// event into CBOR internally and adds it to the queue. If
// CONFIG_HONCH_MAX_QUEUE_DEPTH is reached, the SDK drops the oldest queued
// events to make room for newer ones.
...
// device ID and distinct ID from NVS, restores the MAC-derived device ID,
// resets the distinct ID back to the device ID, and clears all queued events.
...
// flush with a 5-second timeout (blocks until one flush attempt completes or
// the timeout expires), then tears down all subsystems in reverse order: scheduler,
...
| `$battery_low` | Emitted when battery is below `battery_low_threshold` during event tracking. Only fires once while below the threshold, and is armed again after the level recovers to the threshold or above. | `level`: the current battery percentage (int). |
...
| `$device_reset` | Emitted when `honch_reset()` is called, before the queue is cleared and identity is restored to the MAC-derived device ID. | - |

## Error Codes

Most SDK operations return `honch_err_t`. Here's what each value means and when you might see it:
...
| `HONCH_ERR_QUEUE_FULL` | Internal signal used when the RAM queue reaches `CONFIG_HONCH_RAM_QUEUE_DEPTH`; the public tracking path normally spills to NVS or drops the oldest event when the total queue reaches `CONFIG_HONCH_MAX_QUEUE_DEPTH`. |
...
| `HONCH_ERR_TRANSPORT` | Defined in the public enum, but not currently returned by the ESP-IDF public API. Runtime send failures are handled internally with retries or drops, depending on the HTTP status. |
| `HONCH_ERR_TIMEOUT` | Used internally by the synchronous flush path. The final flush in `honch_shutdown()` can time out and log a warning, but teardown still continues. |
...
- **Compression**: When `CONFIG_HONCH_ENABLE_GZIP` is enabled (the default on targets that support the miniz compressor symbol), CBOR payloads at or above `CONFIG_HONCH_GZIP_MIN_BYTES` (default 1024) are gzipped before sending. If gzip fails or doesn't reduce the payload size, raw CBOR is sent instead.
- **Retry with backoff**: If the server returns HTTP 429 or a 5xx error, or the connection fails, the SDK retains the events and retries with exponential backoff: starting at 1 second, doubling up to a maximum of 5 minutes, with +/-25% random jitter on each interval.
- **Auth and rejected request errors**: If the server returns HTTP 401, or another non-429 4xx response, the batch is dropped and not retried. Check the `[honch]` logs if events are disappearing.
