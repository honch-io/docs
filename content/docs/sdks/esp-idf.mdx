...
| `HONCH_ERR_QUEUE_FULL` | The RAM queue reached `CONFIG_HONCH_RAM_QUEUE_DEPTH` before the SDK could spill to NVS. In normal operation the SDK spills to NVS, or drops the oldest event when the total queue reaches `CONFIG_HONCH_MAX_QUEUE_DEPTH`. |
...
| `HONCH_ERR_TRANSPORT` | Reserved for transport setup failures. Runtime send failures are handled internally with retries or drops, depending on the HTTP status. |
| `HONCH_ERR_TIMEOUT` | Reserved for timeout failures. The internal synchronous flush used by `honch_shutdown()` can time out and log a warning, but `honch_shutdown()` still completes teardown. |
