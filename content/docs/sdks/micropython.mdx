...
<Callout type="warn" title="Sync the clock before tracking — the Pico W has no RTC">
...
ntptime.settime()          # set the RTC from NTP (UTC)
```

See [Timestamps](/concepts#timestamps) for how the core treats an unset clock.
...
<Callout type="warn" title="Certificate verification">
The MicroPython port does **not** currently verify the server certificate, unlike the ESP-IDF and Arduino ports (which pin the Google Trust Services roots). Prefer a trusted network path and avoid sending sensitive data over an untrusted one until certificate verification lands. See [Security → Transport](/security#transport-is-tls-by-default).
</Callout>
