...
<Callout title="Status">Stable · `0.3.0`. PyPI package `honch-micropython`. Requires firmware built with the `_honch_core` user C module.</Callout>
...
<Callout title="Certificate verification">
...
try:
    ...
    #  'os_error': 0, 'message': 'API key invalid or revoked', 'component': 'http'}
```

`status` is `'ok'` and `reason` is `'none'` when nothing has failed yet. The SDK also logs the same one-line summary once per distinct failure through the platform log hook. See [Error Context & Diagnostics](/shared-core#error-context--diagnostics).
